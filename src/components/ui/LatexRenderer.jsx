...
export const LatexRenderer = ({ latex, block = false, className = '' }) => {
  ...
  return (
    <div className={cn('relative group', block ? 'my-4' : 'inline-block', className)}>
      <div
        ref={containerRef}
        className={cn(
          'latex-content select-text',
          block &&
            'p-4 bg-gradient-to-br from-violet-500/5 to-cyan-500/5 rounded-lg border border-white/10 overflow-x-auto'
        )}
      />
      <button
        onClick={handleCopy}
...
export const FormulaBlock = ({ label, latex, description }) => {
  return (
    <div className="bg-gradient-to-br from-violet-500/10 to-cyan-500/10 rounded-xl p-5 border border-white/10">
      ...
      {description && <p className="mt-3 text-sm text-gray-400 leading-relaxed">{description}</p>}
    </div>
  )
}
