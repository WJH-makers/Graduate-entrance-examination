        ...
        {filtered.map((item, idx) => (
          <div key={idx} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={cn('px-2 py-1 text-[11px] font-semibold rounded-full', colorMap[item.color] || 'bg-slate-100 text-slate-700')}>
                {item.tag}
              </span>
            </div>
            <p className="text-sm text-slate-700 leading-snug">{item.title}</p>
            <div className="mt-3 flex items-center gap-3 text-xs text-slate-500">
              <button className="px-2 py-1 rounded-lg bg-slate-100 border border-slate-200 hover:border-slate-300">复制要点</button>
              <span className="px-2 py-1 rounded-lg bg-slate-50 border border-dashed border-slate-200">原文链接（待接入爬虫）</span>
            </div>
          </div>
        ))}
      </div>
