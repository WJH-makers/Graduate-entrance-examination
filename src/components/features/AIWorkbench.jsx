...
const AIWorkbench = () => {
  ...
  return (
    <Card className="section-block bg-white">
      ...
      <Tabs defaultValue="predict" className="space-y-4">
        ...
        <TabsContent value="predict">
          ...
          <Button
            ...
            onClick={() =>
              run(
                `请基于最新公开信息，生成考点预测列表（5条要点），面向 ${input}。每条含考法+准备建议，80字内，短横线列出。`
              )
            }
            disabled={loading}
          >
        ...
        <TabsContent value="plan">
          <Section icon={ListChecks} title="弱项/需求描述">
            <Textarea
              placeholder="例：二重积分易失分；TCP 拥塞不会画；作文时间分配…"
              value={input}
          ...
          <Button
            ...
            onClick={() =>
              run(
                `请据此生成3天微计划：${input}。每天列 3-5 个可执行任务，包含时间/材料/达成标准，使用短横线。`
              )
            }
            disabled={loading}
          >
        ...
        <TabsContent value="mnemonic">
          <Section icon={Quote} title="输入知识点（可多条）">
            <Textarea
              placeholder="例：p-test 判别；OPT/LRU 栈性质；旋度梯度=0"
              value={input}
              onChange={(e) => setInput(e.target.value)}
              rows={3}
            />
          ...
          <Button
            ...
            onClick={() =>
              run(
                `为以下考点生成10秒口决和30秒步骤提示：${input}。用短横线列出口决，用数字列步骤。`
              )
            }
            disabled={loading}
          >
        ...
        <TabsContent value="essay">
          ...
          <Button
            ...
            onClick={() =>
              run(
                `请改写并优化以下作文内容：${input}。返回：1) 提纲要点；2) 高分句式3句（含分裂/让步/因果各1）；3) 词汇替换列表。`
              )
            }
            disabled={loading}
          >
        ...
        <TabsContent value="interview">
          <Section icon={MessageSquare} title="复试自我介绍/项目关键词">
            <Textarea
              placeholder="例：自我介绍+项目：CV 检测，C++/PyTorch；兴趣：RL"
              value={input}
              onChange={(e) => setInput(e.target.value)}
              rows={3}
            />
          ...
          <Button
            ...
            onClick={() =>
              run(
                `扮演考官，基于“${input}”生成3轮问答：每轮给1个问题+示范回答要点+改进建议（中文即可）。`
              )
            }
            disabled={loading}
          >
      ...
      {loading && (
        <p className="text-sm text-cyan-700 mt-4 flex items-center gap-2">
          <Loader2 size={14} className="animate-spin" /> 调用 DeepSeek 中…
        </p>
      )}
      {!loading && output && (
