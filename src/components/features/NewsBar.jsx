...
import { Search, RefreshCw, Sparkles, Newspaper, Loader2 } from 'lucide-react';
...
export const NewsBar = () => {
    ...
    useEffect(() => {
        fetchNews(query);
        const timer = setInterval(() => fetchNews(query), 12 * 60 * 60 * 1000); // 每12小时自动刷新
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                            {presets.map(p => (
                                <button
                                    key={p}
                                    onClick={() => { setQuery(p); fetchNews(p); }}
