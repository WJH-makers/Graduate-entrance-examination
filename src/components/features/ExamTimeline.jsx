  ...
  return (
    <section id="timeline" className="mb-10">
      ...
      <div className="relative pl-8">
        <div
          className="absolute left-4 top-0 bottom-0 w-[2px] bg-gradient-to-b from-cyan-400 via-indigo-500 to-violet-500"
          aria-hidden
        />
        <div className="space-y-6">
          {timelineEvents.map((ev, idx) => (
            <div key={idx} className="relative flex gap-4">
              <div
                className="absolute left-2 -translate-x-1.5 mt-1 w-3 h-3 rounded-full bg-white border-2 border-cyan-500 shadow"
                aria-hidden
              />
              ...
              <div className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm p-4 hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3 mb-1">
                  <span
                    className={`px-3 py-1 text-xs font-semibold rounded-full bg-gradient-to-r ${colorMap[ev.color] || 'from-slate-200 to-slate-300'} text-white`}
                  >
                    {ev.tag}
                  </span>
