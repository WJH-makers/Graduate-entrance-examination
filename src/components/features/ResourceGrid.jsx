  ...
  return (
    <Card hover className="flex flex-col h-full group">
          ...
          <div className="flex flex-col gap-1">
            <Badge variant={getCategoryColor(resource.category)}>
              {resource.category === 'Math'
                ? '数学一'
                : resource.category === '408'
                  ? '计算机408'
                  : resource.category === 'English'
                    ? '英语一'
                    : resource.category === 'Politics'
                      ? '政治'
                      : resource.category}
            </Badge>
      ...
      {resource.downloadUrl && resource.downloadUrl !== '#' && (
        <p className="text-[11px] text-slate-500 mb-3 font-mono break-all">
          直链：{resource.downloadUrl}
        </p>
      )}
      ...
      <div className="flex gap-3 mt-auto">
        <Button
          ...
          title={
            resource.downloadUrl && resource.downloadUrl !== '#' ? '在新标签打开资源' : '链接待补充'
          }
          onClick={() => {
