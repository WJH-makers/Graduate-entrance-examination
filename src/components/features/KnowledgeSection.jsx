...
import { Search, BookOpen, ChevronDown, Filter, Brain, Calculator, Lightbulb, Link as LinkIcon, Star, Zap, Terminal, Languages, Scale, AlertTriangle, Clock3 as ClockIcon } from 'lucide-react';
...
const normalizeSection = (section) => {
    const classicProblems = section.classicProblems || (section.classicProblem ? [section.classicProblem] : []);
    const difficulty = ['High', 'Medium', 'Low'].includes(section.difficulty) ? section.difficulty : 'Medium';

    const comparisons = Array.isArray(section.comparisons) ? section.comparisons : [];
    const triggers = Array.isArray(section.triggers) ? section.triggers : (section.triggers ? [section.triggers] : []);
    const timeline = Array.isArray(section.timeline) ? section.timeline : [];
    const memoryTips = Array.isArray(section.memoryTips) ? section.memoryTips : (section.memoryTips ? [section.memoryTips] : []);
    const prerequisites = Array.isArray(section.prerequisites) ? section.prerequisites : (section.prerequisites ? [section.prerequisites] : []);

    const fromRecent = section.recentExams?.map((year, idx) => ({
        year,
        question: `${year} 真题同型：${section.title}`,
        answer: section.examTips?.[0] || classicProblems[idx]?.solution || classicProblems[idx]?.description || section.content?.slice(0, 100)
    }));
    ...
    if (!memoryTips.length) {
        if (triggers.length) autoMemory.push(`看到关键词【${triggers.slice(0, 2).join(' / ')}】立即联想本考点。`);
    ...
    return {
        ...
        secondaryConclusions: section.secondaryConclusions?.length ? section.secondaryConclusions : autoSecondary,
    };
};
...
const KnowledgeSection = () => {
    ...
    const tooManyForDiagram = useMemo(() => (currentSubject?.sections?.length || 0) > 200, [currentSubject]);
    // 如果数据过多，强制回到列表视图
    React.useEffect(() => {
    ...
    const filteredSections = useMemo(() => {
        ...
        return normalizedSections
            .filter(section => {
                const matchCategory = selectedCategory === '全部分类' || section.categoryLabel === selectedCategory;
                const matchSubcategory = selectedSubcategory === '全部子分类' || section.subcategoryLabel === selectedSubcategory;
    ...
    const mermaidChart = useMemo(() => {
        ...
        let chart = 'graph LR\n';
        chart += '    %% Styles\n';
        chart += '    classDef root fill:#8b5cf6,stroke:#7c3aed,stroke-width:2px,color:white,font-weight:bold;\n';
        chart += '    classDef bucket fill:#06b6d4,stroke:#0891b2,stroke-width:2px,color:white,font-weight:bold;\n';
            ...
            <div className="bg-gradient-to-r from-white via-sky-50 to-cyan-50 backdrop-blur-sm rounded-xl p-5 mb-6 border border-slate-200 shadow-lg shadow-cyan-100/60">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        >
                            {categories.map(cat => (
                                <option key={cat} value={cat} className="bg-gray-100 text-gray-900">{cat}</option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        >
                            {subcategories.map(subcat => (
                                <option key={subcat} value={subcat} className="bg-gray-100 text-gray-900">{subcat}</option>
                            ))}
                        </select>
                    </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl p-6 overflow-hidden shadow-sm">
                        ...
                        <p className="text-xs text-slate-500 mt-2">若仍提示语法错误，可切换“列表视图”继续浏览。</p>
                    </div>
                    ) : (
                    sortedSections.length > 0 ? (
                        <>
                            {paginatedSections.map((section) => (
                                <KnowledgeCard key={section.id || `${section.subject}-${section.title}`} section={section} />
                            ))}
                ...
                >
                    <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-3 mb-2">
                            ...
                            <div className="flex gap-2">
                                {section.source && (
                                    <Badge variant="outline" size="sm" className="text-violet-700 border-violet-200 bg-violet-50">
                                        来源：{section.source}
                                    </Badge>
                                )}
                                {section.subject && (
                                ...
                                <Badge variant={section.difficulty === 'High' ? 'danger' : section.difficulty === 'Medium' ? 'warning' : 'success'} size="sm">
                                    {section.difficulty === 'High' ? '高频难点' : section.difficulty === 'Medium' ? '中等难度' : '基础考点'}
                                </Badge>
                                {section.category && (
                                    <Badge variant="default" className="bg-slate-100 text-slate-600 border-slate-200" size="sm">
                                        {t(section.category)}
                                    </Badge>
                                )}
                                ...
                                {section.secondaryConclusions && section.secondaryConclusions.length > 0 && (
                                    <div className="space-y-2">
                                        <div className="flex items-center gap-2 text-amber-700 mb-1">
                                            <Zap size={14} />
                                            <span className="font-bold text-xs uppercase tracking-wider">秒杀结论 (二级结论)</span>
                                        </div>
                                        ...
                                        <div className="bg-white rounded-lg p-4 border border-amber-200">
                                            <ul className="list-disc list-inside space-y-1">
                                                {section.examTips.map((tip, i) => (
                                                    <li key={i} className="text-slate-800 text-sm">{tip}</li>
                                                ))}
                                            </ul>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 space-y-2">
                                        ...
                                        <ul className="list-disc list-inside space-y-1">
                                            {section.memoryTips.map((tip, i) => (
                                                <li key={i} className="text-slate-800 text-sm leading-relaxed">{tip}</li>
                                            ))}
                                        </ul>
                                    </div>
                                )}
                                    ...
                                    <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-4">
                                        ...
                                        <ul className="list-disc list-inside space-y-1">
                                            {section.mistakes.map((mistake, i) => (
                                                <li key={i} className="text-gray-400 text-sm">{mistake}</li>
                                            ))}
                                        </ul>
                                    </div>
                                )}
                                    ...
                                    <div className="bg-amber-500/5 border border-amber-500/10 rounded-lg p-4 space-y-3">
                                        ...
                                        <div className="grid md:grid-cols-2 gap-3">
                                            {section.comparisons.slice(0, 4).map((cmp, idx) => (
                                                <div key={idx} className="bg-white/5 border border-white/10 rounded-md p-3 space-y-1">
                                        ...
                                        <div className="space-y-2">
                                            {section.timeline.slice(0, 5).map((item, idx) => (
                                                <div key={idx} className="flex items-start gap-3">
                                                    <Badge variant="outline" size="xs">{item.year}</Badge>
                                                    <div className="space-y-0.5">
                                                        <p className="text-gray-200 text-sm">{item.event}</p>
                                                        {item.meaning && <p className="text-gray-400 text-xs">意义：{item.meaning}</p>}
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="grid md:grid-cols-2 gap-3">
                                            {section.classicProblems.slice(0, 4).map((problem, idx) => (
                                                <div key={idx} className="bg-white rounded-lg p-4 border border-slate-200 space-y-2 shadow-sm">
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-slate-900 text-sm font-semibold">
                                                            {problem.title || `例题 ${idx + 1}`}
                                                        </span>
                                                        <Badge variant="outline" size="xs" className="text-slate-700 border-slate-300">
                                ...
                                <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                                    ...
                                    <div className="flex gap-2">
                                        {section.relatedPoints.map(pointId => (
                                            <Badge key={pointId} variant="outline" size="xs" className="text-gray-400 border-gray-700">
                                                {pointId}
                                            </Badge>
                                        ))}
                                    </div>
